              ...
              {(data?.quotes ?? []).map((q) => (
                <tr key={q.id} className="border-t border-slate-100">
                  <td className="px-4 py-3 font-medium text-slate-900">
                    {q.name}{" "}
                    <span className="text-xs text-slate-400">({q.symbol})</span>
                  </td>
                  <td className="px-4 py-3 font-semibold text-teal-800">
                    {fmtMoney(q.price, q.currency, q.id === "copper" ? 4 : 2)}
                  </td>
                  <td className="px-4 py-3 text-slate-600">{q.quoteUnit}</td>
                  <td className="px-4 py-3 text-xs text-slate-500">
                    {formatUpdated(q.updatedAt)}
                    <div className="text-[10px] text-slate-400">{q.source}</div>
                  </td>
                </tr>
              ))}
...
function Results({
  ...
  if (!items) {
    return (
      <div className="lg:col-span-2 rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-6 text-sm text-slate-400">
        Enter values to see results (requires a live quote).
      </div>
    );
  }
  return (
    <div className="lg:col-span-2 rounded-2xl border border-teal-200 bg-gradient-to-br from-teal-50 to-white p-6 shadow-sm">
